...
def splitTrainTestSet(X, y, testRatio, randomState=345):
    X_train, X_test, y_train, y_test = train_test_split(X,
                                                        y,
                                                        test_size=testRatio,
                                                        random_state=randomState,#随机数种子
                                                        stratify=y)
...
def test(device, net, test_loader):
    ...
    for inputs, labels in test_loader:
        ...
        if count == 0:
            ...
            y_pred_test = np.concatenate((y_pred_test, outputs))#因为是64 64batch算的，所以要循环结束再把他们链接起来
...
if __name__ == '__main__':
    ...
    # 只保存模型参数
    torch.save(net.state_dict(), '../class_params/SA_MyNet_params.pth')
    ...
    Test_time = toc2 - tic2
    file_name = "../class_result/SA_classification_report.txt"
    with open(file_name, 'w') as x_file:
        ...
        get_class_map.get_cls_map(net, device, all_data_loader, y_all)
